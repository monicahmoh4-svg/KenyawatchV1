...
function getSatelliteUrl(lat, lng, zoom = 17) {
  ...
  // Free fallback tile source (ArcGIS World Imagery) — no key required.
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: rows.map(r => ({ ...r, satellite_image_url: r.satellite_image_url || getSatelliteUrl(r.latitude, r.longitude) })) });
  } catch (e) {
...
router.post('/:id/refresh-satellite', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      'UPDATE ghost_projects SET satellite_image_url=$1, last_satellite_check=NOW() WHERE id=$2 RETURNING *',
      [url, req.params.id]
    );
